...
public interface CompanyBaseInfoDao {
     ...
     */
    @Select(value = {"SELECT user_info.id As companyId,user_info.username as companyName From",COMPANYTABLE,",",AREATABLE,"WHERE area_info.area_no=","#{areaName}","AND area_info.id = user_info.area_id"})
    public List<CompanyBaseInfo> companyBaseInfos(String areaNo);
}
